...
exports = module.exports = function(req, res) {
    ...
    view.on('init', function(next) {
        // Locate this player
        var queryPlayer = Player.model.findOne({ '_id': req.params.id }, {}, {}).populate('team').lean();
        // Game Content
        var querySpecial = SpecialOption.model.find({}, {}, {}).lean()
        // Game Config
        var queryGame = Game.model
					.findOne(
						{},
						{},
						{
        ...
        queryPlayer.exec(function(err, result) {
            ...
            querySpecial.exec(function(err, result) {
                ...
                queryGame.exec(function(err, result) {
                    ...
                    // console.log(result, " Game query result")

                    locals.config = result;

                    // Leaderboard
                    Player.model.find({"leader":{$ne:null}}).lean()
                    .populate('team')
                    .sort('-leader')
                    .exec(function(err, player){
                        ...
                        Team.model.find({}).lean().exec(function(err, team){
